...
function main() {
  ...
  function detectTimesEnv(blockSettings) {
    ...
    if (!nytFonts && nytProject) {
      detected = confirm(
        "You seem to be running ai2html in NYT Preview, but your system is missing the NYT fonts.\nContinue in Preview mode?",
        true
      );
    }
  ...
  function makeVideoHtml(url, settings) {
    ...
    return (
      ...
      '="' +
      url +
      '" autoplay muted loop playsinline style="top:0; width:100%; object-fit:fill; position:absolute"></video>'
    );
  }
  ...
  function addCustomContent(content, customBlocks) {
    if (customBlocks.css) {
      content.css +=
        "\r\t/* Custom CSS */\r\t" + customBlocks.css.join("\r\t") + "\r";
    }
    if (customBlocks["html-before"]) {
      content.html =
        "<!-- Custom HTML -->\r" +
        customBlocks["html-before"].join("\r") +
        "\r" +
        content.html +
        "\r";
    }
    if (customBlocks["html-after"]) {
      content.html +=
        "\r<!-- Custom HTML -->\r" +
        customBlocks["html-after"].join("\r") +
        "\r";
    }
    // deprecated
